(() => {
  ...
  const part2Ready = () => {
    ...
    const ids = ["irii", "lmax", "cable", "cycle"];
  ...
  const viewHome = () => {
    ...
    return `
      <section class="hero">
        <div class="kicker">BTS MS · option A · E4 2025 NC</div>
        <h1>Préparer l’E4 nov. 2025<br>sans attendre le prof.</h1>
        <p>Transstockeurs Kloosterboer, Harnes, entrepôt à −20&nbsp;°C. 4&nbsp;h, coefficient 5. Un parcours calé sur <em>ce</em> sujet : TN-C/S, IR/Ii, câble 95&nbsp;mm², cycle +5&nbsp;%, L10.</p>
        <div class="row">
      ...
      <div class="grid cols-3">
        <article class="card"><div class="tiny">Épreuve</div><div class="stat">4 h</div><p>U4 Intégration d’un bien · coef. 5</p></article>
        <article class="card"><div class="tiny">Cœur du sujet</div><div class="stat">120 min</div><p>Partie 1 : TN, disjoncteur, câble</p></article>
      ...
      <div class="card" style="margin-top:1rem">
        ...
        <div class="list-item"><div><strong>1 · Fiches</strong><div class="tiny">Une méthode, un piège NC 2025, un exemple</div></div><a class="btn sm ghost" href="#/fiches">Ouvrir</a></div>
  ...
  const viewDiagnostic = () => {
    ...
    const i = Number(sessionStorage.getItem("diag-i-2025-nc") || 0);
  ...
  const bindDiagnostic = () => {
    ...
    if (redo) {
      redo.onclick = () => {
        ...
        sessionStorage.setItem("diag-i-2025-nc", "0");
        save();
        render();
      };
      return;
    }
    let i = Number(sessionStorage.getItem("diag-i-2025-nc") || 0);
    ...
    const go = (n) => {
      sessionStorage.setItem("diag-i-2025-nc", String(n));
      render();
    };
    ...
    <div class="grid cols-2">
      ${FICHES.map((f) => `
        <a class="card" href="#/fiches/${f.id}" style="color:inherit">
          ...
          <h3>${esc(f.title)}</h3>
          <p class="tiny">${state.fiches[f.id] ? "Déjà ouverte" : "À lire"}</p>
        </a>`).join("")}
    </div>`;
  ...
  const viewExercices = () => `
    <p class="muted">Même physique que le sujet NC 2025, parfois les vrais nombres, parfois un décalage pour éviter le par cœur.</p>
    ${SKILLS.map((s) => {
  ...
  const viewDqrList = () => {
    ...
    return `
      ${warn ? `<div class="card" style="margin-bottom:1rem;border-color:#8a5a00">
        <strong>Conseil.</strong> Le diagnostic IR/Ii / câble / cycle n’est pas encore assez solide. Vous pouvez ouvrir le DQR, mais la partie 1 (120 min) risque de vous bloquer. Mieux : fiche disjoncteur + fiche K3 + 3 exercices.
        <div class="actions"><a class="btn sm" href="#/fiches/irii">Fiche IR / Ii</a></div>
      </div>` : ""}
      <p class="muted">Dossier d’entraînement calé sur le sujet NC 2025 — pas une copie du DQR officiel. Gardez le PDF à côté pour les schémas DT.</p>
      <div class="grid cols-2">
        ${DQR.map((p) => {
  ...
  const viewBlanc = () => {
    ...
    if (!b.start && !b.submitted) {
      ...
      return `<div class="paper">
        <h2>Blanc E4 2025 NC — conditions d’examen</h2>
        ...
        <table>
          ...
          <tr><td>1-1 / 1-2 TN + IR/Ii</td><td>50 min</td><td>125 A, 200 A, 750 A — pas 160/2200</td></tr>
          <tr><td>1-3 Lmax</td><td>20 min</td><td>V = 230 V, Ii = 2250 A</td></tr>
          <tr><td>1-4 Câble</td><td>40 min</td><td>Lettre E, K3 = 0,71, pas −20 °C</td></tr>
          <tr><td>2 Performances</td><td>100 min</td><td>FA = 30 kN ; 31,5 s = +5 %</td></tr>
          <tr><td>3 Joints</td><td>10 min</td><td>0,045 mm ≪ 0,2 mm</td></tr>
          <tr><td>Relecture</td><td>20 min</td><td>t → kg ; 400 V ≠ V de Lmax</td></tr>
        </table>
        ...
        <div class="actions">
          ...
          <a class="btn secondary" href="assets/E4-2025-NC.pdf" target="_blank">Ouvrir le PDF officiel</a>
        </div>
      </div>`;
    }
  ...
  const viewTransfert = () => `
    <div class="hero">
      ...
      <p>Si vous réussissez le blanc NC 2025, vous avez appris <em>un</em> sujet. Le jury changera le support. Ce qui reste : lire le DT, isoler un modèle, conclure.</p>
    </div>
    <div class="grid cols-2">
    ...
    <div class="card" style="margin-top:1rem">
      <p>Après cette annale, ouvrez le parcours 2025 (transbordeur) ou 2024 (presse) : la méthode est la même, le métier change. Ne mémorisez pas 750 A par cœur.</p>
    </div>`;

  const viewFormules = () => `
    <article class="paper">
      <h2>Carnet de formules — E4 2025 NC</h2>
      <table>
        <tr><th>Besoin</th><th>Formule</th></tr>
        <tr><td>Seuils disjoncteur</td><td><span class="formule">I<sub>R</sub> = 0,8 I<sub>n</sub></span> · <span class="formule">I<sub>i</sub> = 3 I<sub>n</sub></span> (réglages du sujet)</td></tr>
        <tr><td>Longueur max câble</td><td><span class="formule">L<sub>max</sub> = 0,8 V S<sub>ph</sub> / ((1+m) ρ I<sub>i</sub>)</span> · V = 230 V</td></tr>
        <tr><td>Câble</td><td><span class="formule">I<sub>b</sub> ≤ I<sub>R</sub> ≤ I<sub>z</sub></span> · <span class="formule">I′<sub>z</sub> = I<sub>z</sub> · K</span> · K = K1 K2 K3</td></tr>
        <tr><td>Poids</td><td><span class="formule">P = m g</span> (5 t = 5000 kg, g = 10)</td></tr>
        <tr><td>PFS</td><td>Σ F⃗ = 0⃗ et Σ M⃗ = 0⃗</td></tr>
        <tr><td>Galet</td><td><span class="formule">V = r ω</span> · <span class="formule">N = 30 ω / π</span></td></tr>
        <tr><td>Adhérence</td><td><span class="formule">|T| ≤ μ N</span></td></tr>
        <tr><td>Profil acc / V / déc</td><td><span class="formule">t = V/a</span> · <span class="formule">X = ½ a t²</span></td></tr>
        <tr><td>L10 / Lh</td><td><span class="formule">L<sub>10</sub> = (C/P)<sup>k</sup></span> · <span class="formule">L<sub>h</sub> = 10⁶ L<sub>10</sub> / (60 N)</span></td></tr>
      </table>
  ...
  const viewProf = () => `
    <article class="paper">
      ...
      <p>Ce site est conçu pour tourner tout seul en salle info ou à la maison. Votre plus-value : le schéma TN au tableau, le piège Ii = 750 A, et le profil 31,5 s = +5 %.</p>
      <h3>Usage type</h3>
      <ol>
        <li>Semaine 1 : 20 min de présentation de l’entrepôt Harnes (−20 °C), puis diagnostic en silence.</li>
        <li>Les étudiants suivent les pastilles rouges. Vous ne recalculez plus Lmax à la chaîne.</li>
        <li>Une séance salle sur DT3 (lire Ib / IR / Ii) et une sur DT8 (cycle actuel vs nouveau).</li>
        <li>Blanc surveillé (PDF officiel + site, ou PDF seul).</li>
        <li>30 min de copies types, puis transfert 2025 métropole (même PFS, autre métier).</li>
      </ol>
      <h3>Trois pièges à traiter en classe</h3>
      <p><strong>Ii = 750 A fait sauter au démarrage.</strong> IR = 200 A ne voit pas +20 % (150 A). L’ancien Ii = 2200 A laissait passer ; 3 In est trop serré. Lmax s’enchaîne avec Ii = 2250 A.</p>
      <p><strong>K3 = 0,71 à 45 °C, pas à −20 °C.</strong> Lettre E, 95 mm² → I′z ≈ 183 A &lt; IR = 200 A → passer en 120 mm². V dans Lmax = 230 V, pas 400 V.</p>
      <p><strong>31,5 s = 30 s + 5 % pile.</strong> On est sur la limite production, donc conforme. Ne pas comparer 63 s au cycle 70 s (celui-ci contient la pause).</p>
      <h3>Hébergement</h3>
      <p>Depuis la racine du dépôt :</p>
      <p><span class="formule">python -m http.server 5500</span> puis ouvrir <span class="formule">http://127.0.0.1:5500/e4/2025-nc/</span></p>
      <p>La progression est stockée dans le navigateur de l’élève, clé <span class="formule">prepa-e4-2025-nc-v1</span> (séparée des autres annales). En salle, prévoir un poste par étudiant ou un profil navigateur.</p>
  ...
  function render() {
    ...
    $title.textContent = titles[path] || "Prépa E4 2025 NC";
    drawNav(path);
    closeMenu();
