(() => {
  ...
  const part2Ready = () => {
    ...
    const ids = ["courroie", "huile", "kp", "mesure"];
  ...
  const viewHome = () => {
    ...
    return `
      <section class="hero">
        <div class="kicker">BTS MS · option A · E4 2023</div>
        <h1>Préparer l’E4 2023<br>sans attendre le prof.</h1>
        <p>Centre de pétrissage Pasquier, Étoile-sur-Rhône. 4&nbsp;h, coefficient 6. Un parcours calé sur <em>ce</em> sujet : classes d’équivalence, courroie SPA, huile Rossi, Kp AGMA, 4–20&nbsp;mA.</p>
        <div class="row">
      ...
      <div class="grid cols-3">
        ...
        <article class="card"><div class="tiny">Cœur du sujet</div><div class="stat">90 min</div><p>Partie 4 : température, quantum, API</p></article>
      ...
      <div class="card" style="margin-top:1rem">
        ...
        <div class="list-item"><div><strong>1 · Fiches</strong><div class="tiny">Une méthode, un piège 2023, un exemple</div></div><a class="btn sm ghost" href="#/fiches">Ouvrir</a></div>
  ...
  const viewDiagnostic = () => {
    ...
    const i = Number(sessionStorage.getItem("diag-i-2023") || 0);
  ...
  const bindDiagnostic = () => {
    ...
    if (redo) {
      redo.onclick = () => {
        ...
        sessionStorage.setItem("diag-i-2023", "0");
        save();
        render();
      };
      return;
    }
    let i = Number(sessionStorage.getItem("diag-i-2023") || 0);
    ...
    const go = (n) => {
      sessionStorage.setItem("diag-i-2023", String(n));
      render();
    };
  ...
  const viewExercices = () => `
    <p class="muted">Même physique que 2023, parfois les vrais nombres, parfois un décalage pour éviter le par cœur.</p>
    ${SKILLS.map((s) => {
  ...
  const viewDqrList = () => {
    ...
    return `
      ${warn ? `<div class="card" style="margin-bottom:1rem;border-color:#8a5a00">
        <strong>Conseil.</strong> Le diagnostic courroie / huile / mesure n’est pas encore assez solide. Vous pouvez ouvrir le DQR, mais les parties 2 à 4 risquent de vous bloquer. Mieux : fiche LAC + fiche 4–20 mA + 3 exercices.
        <div class="actions"><a class="btn sm" href="#/fiches/courroie">Fiche courroie</a></div>
      </div>` : ""}
      <p class="muted">Dossier d’entraînement calé sur le sujet 2023 — pas une copie du DQR officiel. Gardez le PDF à côté pour les schémas DT.</p>
      <div class="grid cols-2">
        ${DQR.map((p) => {
  ...
  const viewBlanc = () => {
    ...
    if (!b.start && !b.submitted) {
      ...
      return `<div class="paper">
        <h2>Blanc E4 2023 — conditions d’examen</h2>
        ...
        <table>
          ...
          <tr><td>1 Cinématique</td><td>40 min</td><td>DT5+DT6 avant de colorier DR1</td></tr>
          <tr><td>2 Courroie</td><td>40 min</td><td>LAC puis SPA 2650, pas 2582</td></tr>
          <tr><td>3 Huile + Compabloc</td><td>70 min</td><td>1,2 L est B3 ; Kp ≥ 1,4</td></tr>
          <tr><td>4 Température / API</td><td>80 min</td><td>I = 4+16(T−20)/80 ; %IW105.3</td></tr>
          <tr><td>Relecture</td><td>10 min</td><td>62/68 °C ≠ 18/102 °C ; 2 courroies</td></tr>
        </table>
        ...
        <div class="actions">
          ...
          <a class="btn secondary" href="assets/E4-2023.pdf" target="_blank">Ouvrir le PDF officiel</a>
        </div>
      </div>`;
    }
  ...
  const viewTransfert = () => `
    <div class="hero">
      ...
      <p>Si vous réussissez le blanc 2023, vous avez appris <em>un</em> sujet. Le jury changera le support. Ce qui reste : lire le DT, isoler un modèle, conclure.</p>
    </div>
    <div class="grid cols-2">
    ...
    <div class="card" style="margin-top:1rem">
      <p>Après cette annale, ouvrez le parcours 2025 (transbordeur) ou 2024 (presse) : la méthode est la même, le métier change. Ne mémorisez pas SPA 2650 par cœur.</p>
    </div>`;

  const viewFormules = () => `
    <article class="paper">
      <h2>Carnet de formules — E4 2023</h2>
      <table>
        <tr><th>Besoin</th><th>Formule</th></tr>
        <tr><td>Longueur courroie</td><td><span class="formule">LAC = 2E + 1,57(D+d) + (D−d)² / (4E)</span> (mm)</td></tr>
        <tr><td>Choix ISO</td><td>Longueur SPA la plus proche, autant de brins que de gorges</td></tr>
        <tr><td>Huile Rossi</td><td>Taille × position réelle (B3 / V6 / V5), pas B5 moteur</td></tr>
        <tr><td>Kp AGMA</td><td>Classe I → 1 · II → 1,4 · III → 2</td></tr>
        <tr><td>Choix Compabloc</td><td>Kp choisi ≥ Kp requis, puis Ns la plus proche</td></tr>
        <tr><td>4–20 mA (20–100 °C)</td><td><span class="formule">I = 4 + 16 (T − 20) / 80</span></td></tr>
        <tr><td>Quantum n bits</td><td><span class="formule">q = Δ / (2ⁿ − 1)</span> (DT20)</td></tr>
        <tr><td>Mot 12 bits</td><td><span class="formule">N = (I − 4) / 16 × 4095</span></td></tr>
        <tr><td>Limites module</td><td><span class="formule">Min/Max ± 5 % × (Max − Min) / 2</span></td></tr>
        <tr><td>Mot image AEY800</td><td><span class="formule">%IWxy.i</span> (ex. %IW105.3)</td></tr>
      </table>
  ...
  const viewProf = () => `
    <article class="paper">
      ...
      <p>Ce site est conçu pour tourner tout seul en salle info ou à la maison. Votre plus-value : le plan DT5 au tableau, le décodage Rossi, et la droite 4–20 mA.</p>
      <h3>Usage type</h3>
      <ol>
        <li>Semaine 1 : 20 min de présentation du centre Pasquier, puis diagnostic en silence.</li>
        <li>Les étudiants suivent les pastilles rouges. Vous ne recalculez plus LAC à la chaîne.</li>
        <li>Une séance salle sur DR1 (classes) et une sur DT11 (huile : 1,2 L vs 1,7 L).</li>
        <li>Blanc surveillé (PDF officiel + site, ou PDF seul).</li>
        <li>30 min de copies types, puis transfert 2025 (même 4–20 mA, autre métier).</li>
      </ol>
      <h3>Trois pièges à traiter en classe</h3>
      <p><strong>1,2 L n’est pas le bon volume.</strong> C’est B3 taille 5. Le motoréducteur 10 est en V6 → 1,7 L. B5 dans la référence est la bride moteur.</p>
      <p><strong>SPA 2582 n’est pas la courroie à commander.</strong> LAC ≈ 2664 mm → SPA 2650, deux brins (2G). 2582 est le repère 8.</p>
      <p><strong>62/68 °C ≠ 18/102 °C.</strong> Les premiers sont la consigne process (65 ± 3). Les seconds sont les limites du module AEY800. Mot image : %IW105.3, pas une sortie %QW.</p>
      <h3>Hébergement</h3>
      <p>Depuis la racine du dépôt :</p>
      <p><span class="formule">python -m http.server 5500</span> puis ouvrir <span class="formule">http://127.0.0.1:5500/e4/2023/</span></p>
      <p>La progression est stockée dans le navigateur de l’élève, clé <span class="formule">prepa-e4-2023-v1</span> (séparée de 2024 et 2025). En salle, prévoir un poste par étudiant ou un profil navigateur.</p>
  ...
  function render() {
    ...
    $title.textContent = titles[path] || "Prépa E4 2023";
    drawNav(path);
    closeMenu();
