(() => {
  const { SKILLS, DATA_2025, DIAGNOSTIC, FICHES, EXERCICES, DQR, TRANSFERT, CONTRAT } = window.PREPA;
  const KEY = "prepa-e4-2025-v1";
  ...
  const part2Ready = () => {
    ...
    const ids = ["statique", "incline", "couple", "frein"];
  ...
  const viewHome = () => {
    ...
    return `
      <section class="hero">
        <div class="kicker">BTS MS · option A · E4 2025</div>
        <h1>Préparer l’E4 2025<br>sans attendre le prof.</h1>
        <p>Transbordeur de la ligne de peinture, Stellantis Sochaux. 4&nbsp;h, coefficient 5. Un parcours calé sur <em>ce</em> sujet, jusqu’au blanc chrono.</p>
        <div class="row">
      ...
      <div class="grid cols-3">
        ...
        <article class="card"><div class="tiny">Cœur mécanique</div><div class="stat">90 min</div><p>Partie 2 : plan 12°, doigts, frein</p></article>
      ...
      <div class="card" style="margin-top:1rem">
        ...
        <div class="list-item"><div><strong>1 · Fiches</strong><div class="tiny">Une méthode, un piège 2025, un exemple</div></div><a class="btn sm ghost" href="#/fiches">Ouvrir</a></div>
  ...
  const viewDiagnostic = () => {
    ...
    const i = Number(sessionStorage.getItem("diag-i") || 0);
  ...
  const bindDiagnostic = () => {
    ...
    if (redo) {
      redo.onclick = () => {
        ...
        sessionStorage.setItem("diag-i", "0");
        save();
        render();
      };
      return;
    }
    let i = Number(sessionStorage.getItem("diag-i") || 0);
    ...
    const go = (n) => {
      sessionStorage.setItem("diag-i", String(n));
      render();
    };
  ...
  const viewExercices = () => `
    <p class="muted">Même physique que 2025, parfois les vrais nombres, parfois un décalage pour éviter le par cœur.</p>
    ${SKILLS.map((s) => {
  ...
  const viewDqrList = () => {
    ...
    return `
      ${warn ? `<div class="card" style="margin-bottom:1rem;border-color:#8a5a00">
        <strong>Conseil.</strong> Le diagnostic partie 2 n’est pas encore assez solide. Vous pouvez ouvrir le DQR, mais la partie 2-1 risque de vous bloquer. Mieux : fiche plan incliné + 3 exercices.
        <div class="actions"><a class="btn sm" href="#/fiches/incline">Fiche plan incliné</a></div>
      </div>` : ""}
      <p class="muted">Dossier d’entraînement calé sur le sujet 2025 — pas une copie du DQR officiel. Gardez le PDF à côté pour les schémas DT.</p>
      <div class="grid cols-2">
        ${DQR.map((p) => {
  ...
  const viewBlanc = () => {
    ...
    if (!b.start && !b.submitted) {
      ...
      return `<div class="paper">
        <h2>Blanc E4 2025 — conditions d’examen</h2>
        ...
        <table>
          ...
          <tr><td>1 Analyse</td><td>25 min</td><td>Passer, y revenir</td></tr>
          <tr><td>2-1 / 2-2 Mécanique</td><td>80 min</td><td>Ne pas rester 40 min sur un schéma</td></tr>
          <tr><td>3 Cinématique + ATV</td><td>55 min</td><td>Q.3-1 à 3-4 d’abord</td></tr>
          <tr><td>4 Mesure / API</td><td>55 min</td><td>Tracer les droites avant les calculs</td></tr>
          <tr><td>Relecture</td><td>15 min</td><td>Unités, rayon, pauses</td></tr>
        </table>
        ...
        <div class="actions">
          ...
          <a class="btn secondary" href="assets/E4-2025.pdf" target="_blank">Ouvrir le PDF officiel</a>
        </div>
      </div>`;
    }
  ...
  const viewTransfert = () => `
    <div class="hero">
      ...
      <p>Si vous réussissez le blanc 2025, vous avez appris <em>un</em> sujet. Le jury de l’année suivante changera le support. Ce qui reste : lire le DT, isoler un modèle, conclure.</p>
    </div>
    <div class="grid cols-2">
    ...
    <div class="card" style="margin-top:1rem">
      <p>Les PDF des autres sessions sont dans le dossier enseignant <span class="formule">eval-etudiants/annales/E4-SP/</span>. Après une annale, revenez ici seulement pour les fiches méthode — pas pour le corrigé 2025.</p>
    </div>`;

  const viewFormules = () => `
    <article class="paper">
      <h2>Carnet de formules — E4 2025</h2>
      <table>
        <tr><th>Besoin</th><th>Formule</th></tr>
        <tr><td>Poids</td><td><span class="formule">P = m g</span> (g = 10)</td></tr>
        <tr><td>Adhérence (démarrage)</td><td><span class="formule">F_adh = μ_adh N</span></td></tr>
        <tr><td>Frottement (glissement)</td><td><span class="formule">F_frot = μ_frot N</span></td></tr>
        <tr><td>Plan incliné</td><td><span class="formule">P∥ = P sin α</span> · <span class="formule">N = P cos α</span></td></tr>
        <tr><td>Force doigt</td><td><span class="formule">F = P sin α − μ_frot N</span></td></tr>
        <tr><td>Couple</td><td><span class="formule">C = F · r</span> (r en mètres)</td></tr>
        <tr><td>Remontée de couple</td><td><span class="formule">C_s = C_e · i · η</span> puis comparer sur le même arbre</td></tr>
        <tr><td>Pignon-crémaillère</td><td><span class="formule">V = r ω</span> · <span class="formule">ω = 2π N / 60</span></td></tr>
        <tr><td>Variateur</td><td><span class="formule">f / 50 = N / N_50Hz</span></td></tr>
        <tr><td>4–20 mA (1,0 → 2,5 m)</td><td><span class="formule">I = 4 + 16 (L − 1) / 1,5</span></td></tr>
        <tr><td>CAN configuré 0–1023</td><td><span class="formule">N = 1023 (L − 1) / 1,5</span></td></tr>
      </table>
      <h3>Données du sujet à avoir sous les yeux</h3>
      <table>${DATA_2025.map((r) => `<tr><td>${esc(r[0])}</td><td>${esc(r[1])}</td></tr>`).join("")}</table>
    </article>`;

  const viewProf = () => `
    <article class="paper">
      <h2>Espace enseignant</h2>
      <p>Ce site est conçu pour tourner tout seul en salle info ou à la maison. Votre plus-value : le schéma de forces au tableau et les phrases de conclusion.</p>
      <h3>Usage type</h3>
      <ol>
        <li>Semaine 1 : 20 min de présentation du transbordeur, puis diagnostic en silence.</li>
        <li>Les étudiants suivent les pastilles rouges. Vous ne corrigez plus les 6800 N à la chaîne.</li>
        <li>Une séance salle sur Q.2-1 (schéma). Le reste en autonomie.</li>
        <li>Blanc surveillé (PDF officiel + site en mode blanc, ou PDF seul).</li>
        <li>30 min de copies types, puis transfert 2024.</li>
      </ol>
      <h3>Interprétation frein (DT5)</h3>
      <p>Le catalogue SEW affiche un <em>couple de sortie 50 Hz = 265 N·m</em> (capacité du réducteur) et un <em>frein 400 AC / 5 N·m</em> (arbre moteur). La chaîne Q.2-2 part de <strong>5 N·m</strong>. Si un élève prend 265, toute la suite dérape — c’est le piège à traiter en classe.</p>
      <h3>Hébergement</h3>
      <p>Double-cliquer <span class="formule">serveur.bat</span> ou, dans ce dossier :</p>
      <p><span class="formule">python -m http.server 5500</span> puis ouvrir <span class="formule">http://127.0.0.1:5500</span></p>
      <p>La progression est stockée dans le navigateur de l’élève (pas de comptes). En salle, prévoir un poste par étudiant ou un profil navigateur.</p>
  ...
  function render() {
    ...
    $title.textContent = titles[path] || "Prépa E4 2025";
    drawNav(path);
    closeMenu();
