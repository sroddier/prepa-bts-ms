(() => {
  ...
  const part2Ready = () => {
    ...
    const ids = ["hydro", "debit", "force", "porte"];
  ...
  const viewHome = () => {
    ...
    return `
      <section class="hero">
        <div class="kicker">BTS MS · option A · E4 2024</div>
        <h1>Préparer l’E4 2024<br>sans attendre le prof.</h1>
        <p>Presse à compacter ALLTUB, Saumur. 4&nbsp;h, coefficient 5. Un parcours calé sur <em>ce</em> sujet : hydraulique, porte, câble, barrière, réseau.</p>
        <div class="row">
      ...
      <div class="grid cols-3">
        ...
        <article class="card"><div class="tiny">Cœur du sujet</div><div class="stat">70 min</div><p>Partie 1 : hydraulique, cycle, force</p></article>
      ...
      <div class="card" style="margin-top:1rem">
        ...
        <div class="list-item"><div><strong>1 · Fiches</strong><div class="tiny">Une méthode, un piège 2024, un exemple</div></div><a class="btn sm ghost" href="#/fiches">Ouvrir</a></div>
  ...
  const viewDiagnostic = () => {
    ...
    const i = Number(sessionStorage.getItem("diag-i-2024") || 0);
  ...
  const bindDiagnostic = () => {
    ...
    if (redo) {
      redo.onclick = () => {
        ...
        sessionStorage.setItem("diag-i-2024", "0");
        save();
        render();
      };
      return;
    }
    let i = Number(sessionStorage.getItem("diag-i-2024") || 0);
    ...
    const go = (n) => {
      sessionStorage.setItem("diag-i-2024", String(n));
      render();
    };
  ...
  const viewExercices = () => `
    <p class="muted">Même physique que 2024, parfois les vrais nombres, parfois un décalage pour éviter le par cœur.</p>
    ${SKILLS.map((s) => {
  ...
  const viewDqrList = () => {
    ...
    return `
      ${warn ? `<div class="card" style="margin-bottom:1rem;border-color:#8a5a00">
        <strong>Conseil.</strong> Le diagnostic hydraulique / force n’est pas encore assez solide. Vous pouvez ouvrir le DQR, mais la partie 1 risque de vous bloquer. Mieux : fiche Q = V·S + 3 exercices.
        <div class="actions"><a class="btn sm" href="#/fiches/debit">Fiche débits</a></div>
      </div>` : ""}
      <p class="muted">Dossier d’entraînement calé sur le sujet 2024 — pas une copie du DQR officiel. Gardez le PDF à côté pour les schémas DT.</p>
      <div class="grid cols-2">
        ${DQR.map((p) => {
  ...
  const viewBlanc = () => {
    ...
    if (!b.start && !b.submitted) {
      ...
      return `<div class="paper">
        <h2>Blanc E4 2024 — conditions d’examen</h2>
        ...
        <table>
          ...
          <tr><td>1 Hydraulique + cycle</td><td>65 min</td><td>Relever pressions et débits d’abord</td></tr>
          <tr><td>2 Porte</td><td>30 min</td><td>Section annulaire, pas πd²/4</td></tr>
          <tr><td>3 Électrique</td><td>65 min</td><td>Pt=22 kW / Qt=14 kVAr donnés ensuite</td></tr>
          <tr><td>4 Barrière</td><td>45 min</td><td>S = 2000 T + 8(d−14)</td></tr>
          <tr><td>5 Réseau</td><td>20 min</td><td>.0 et .255 interdits</td></tr>
          <tr><td>Relecture</td><td>15 min</td><td>bar → Pa, L/min → m³/s, mm → m</td></tr>
        </table>
        ...
        <div class="actions">
          ...
          <a class="btn secondary" href="assets/E4-2024.pdf" target="_blank">Ouvrir le PDF officiel</a>
        </div>
      </div>`;
    }
  ...
  const viewTransfert = () => `
    <div class="hero">
      ...
      <p>Si vous réussissez le blanc 2024, vous avez appris <em>un</em> sujet. Le jury changera le support. Ce qui reste : lire le DT, isoler un modèle, conclure.</p>
    </div>
    <div class="grid cols-2">
    ...
    <div class="card" style="margin-top:1rem">
      <p>Après cette annale, ouvrez le parcours 2025 (transbordeur) : la méthode est la même, le métier change. Ne mémorisez pas 560 kN par cœur.</p>
    </div>`;

  const viewFormules = () => `
    <article class="paper">
      <h2>Carnet de formules — E4 2024</h2>
      <table>
        <tr><th>Besoin</th><th>Formule</th></tr>
        <tr><td>Section piston</td><td><span class="formule">S = π d² / 4</span> (d en mètres)</td></tr>
        <tr><td>Débit / vitesse</td><td><span class="formule">Q = V · S</span> donc <span class="formule">V = Q / S</span></td></tr>
        <tr><td>L/min → m³/s</td><td><span class="formule">Q = q_L / 60 000</span></td></tr>
        <tr><td>Force vérin</td><td><span class="formule">F = p · S</span> avec 1 bar = 10⁵ Pa</td></tr>
        <tr><td>Section annulaire</td><td><span class="formule">S = π (D² − d²) / 4</span></td></tr>
        <tr><td>Masse / poids</td><td><span class="formule">m = ρ V</span> · <span class="formule">P = m g</span> (ici g = 9,81)</td></tr>
        <tr><td>Puissance active</td><td><span class="formule">P = √3 U I cos φ</span></td></tr>
        <tr><td>Puissance réactive</td><td><span class="formule">Q = √3 U I sin φ</span></td></tr>
        <tr><td>Apparente / courant</td><td><span class="formule">S = √(P² + Q²)</span> · <span class="formule">I = S / (√3 U)</span></td></tr>
        <tr><td>Seuil long retard</td><td><span class="formule">I_lr = I_n · I_o · I_r</span></td></tr>
        <tr><td>Distance barrière</td><td><span class="formule">S = 2000 T + 8 (d − 14)</span> (mm, T en s)</td></tr>
      </table>
  ...
  const viewProf = () => `
    <article class="paper">
      ...
      <p>Ce site est conçu pour tourner tout seul en salle info ou à la maison. Votre plus-value : le schéma hydraulique au tableau et le raisonnement « section annulaire ».</p>
      <h3>Usage type</h3>
      <ol>
        <li>Semaine 1 : 20 min de présentation de la presse ALLTUB, puis diagnostic en silence.</li>
        <li>Les étudiants suivent les pastilles rouges. Vous ne recalculez plus πd²/4 à la chaîne.</li>
        <li>Une séance salle sur DT2 (deux pompes + Y14) et sur la section annulaire.</li>
        <li>Blanc surveillé (PDF officiel + site, ou PDF seul).</li>
        <li>30 min de copies types, puis transfert 2025.</li>
      </ol>
      <h3>Trois pièges à traiter en classe</h3>
      <p><strong>220 bar ≠ 285 bar.</strong> La pression de compactage dans le vérin est celle de B4 (220 bar), pas le tarage R1/R2 (285 bar).</p>
      <p><strong>Deux débits = 132 et 37 L/min</strong> (les deux pompes, puis la petite seule via Y14). Pas 95 et 37 pris séparément sans le schéma.</p>
      <p><strong>Levage en entrée de tige :</strong> S = π(ØAL² − ØMM²)/4 avec ØMM = 45 mm. Qui prend π×63²/4 trouve un vérin « trop juste » ou se trompe de facteur 2.</p>
      <h3>Hébergement</h3>
  ...
  function render() {
    ...
    $title.textContent = titles[path] || "Prépa E4 2024";
    drawNav(path);
    closeMenu();
